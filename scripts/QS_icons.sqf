...
Version:
	...
	2.0 (released 3/07/2015 A3 v1.46)
...
Last Modified: 
	...
	3/07/2015 ArmA 3 1.46 by Quiksilver
...
_QS_ST_showAll = FALSE;									// BOOL. Set TRUE to reveal all units. Good for Debug/etc. Will reveal enemy. Default FALSE.
...
_QS_fnc_groupIconText = {
	...
	if (_di isEqualTo 1) then {
		if ((_QS_ST_X select 36)) then {
			if ((_QS_ST_X select 73)) then {
				if ((side _grp) isEqualTo playerSide) then {
					if ((_QS_ST_X select 35)) then {
						if (!isNil {_grp getVariable 'BIS_dynamicGroups_groupId'}) then {
							if (!isNil 'BIS_dynamicGroups_list') then {
								_grpIndex = 0;
								if ((_QS_ST_X select 3) isEqualTo 0) then {_grpIndex = 1;};
								if ((_QS_ST_X select 3) isEqualTo 1) then {_grpIndex = 0;};
								if ((_QS_ST_X select 3) isEqualTo 2) then {_grpIndex = 2;};
								_index = -1;
								_index = [(BIS_dynamicGroups_list select _grpIndex),(_grp getVariable 'BIS_dynamicGroups_groupId'),0] call (_QS_ST_X select 77);
								if (!(_index isEqualTo -1)) exitWith {
									_text = ((BIS_dynamicGroups_list select _grpIndex) select _index) select 1;
								};
								_text = groupId _grp; 
							};
						} else {
				...
				if (((side _grp) isEqualTo playerSide) || {((playerSide getFriend (side _grp)) > 0.6)}) then {
					if ((_QS_ST_X select 35)) then {
						if (!isNil {_grp getVariable 'BIS_dynamicGroups_groupId'}) then {
							if (!isNil 'BIS_dynamicGroups_list') then {
								_grpIndex = 0;
...
_QS_fnc_getIndex = {
	// Credit: Zenophon (ZEN_fnc_arrayGetNestedIndex)
	private [
		'_givenSearchValue','_givenSearchArray','_desiredIndex','_index','_nestedArray','_currentIndex'
	];
...
QS_ST_R = [
	_QS_ST_map_enableUnitIcons,_QS_ST_gps_enableUnitIcons,	_QS_ST_enableGroupIcons,_QS_ST_faction,_QS_ST_friendlySides_EAST,
	_QS_ST_friendlySides_WEST,_QS_ST_friendlySides_RESISTANCE,_QS_ST_friendlySides_CIVILIAN,_QS_ST_friendlySides_Dynamic,_QS_ST_iconColor_EAST,
	_QS_ST_iconColor_WEST,_QS_ST_iconColor_RESISTANCE,_QS_ST_iconColor_CIVILIAN,_QS_ST_iconColor_UNKNOWN,_QS_ST_showMedicalWounded,
	_QS_ST_MedicalSystem,_QS_ST_MedicalIconColor,_QS_ST_iconShadowMap,_QS_ST_iconShadowGPS,_QS_ST_iconTextSize_Map,
	_QS_ST_iconTextSize_GPS,_QS_ST_iconTextOffset,_QS_ST_iconSize_Man,_QS_ST_iconSize_LandVehicle,_QS_ST_iconSize_Ship,
	_QS_ST_iconSize_Air,_QS_ST_iconSize_StaticWeapon,_QS_ST_GPSDist,_QS_ST_GPSshowNames,_QS_ST_GPSshowGroupOnly,
	_QS_ST_showAIGroups,_QS_ST_showGroupMapIcons,_QS_ST_showGroupHudIcons,_QS_ST_groupInteractiveIcons,_QS_ST_groupInteractiveIcons_showClass,
	_QS_ST_dynamicGroupID,_QS_ST_showGroupMapText,_QS_ST_groupIconScale,_QS_ST_groupIconOffset,_QS_ST_groupIconText,_QS_ST_autonomousVehicles,
	_QS_fnc_iconColor,_QS_fnc_iconType,_QS_fnc_iconSize,_QS_fnc_iconPosDir,_QS_fnc_iconText,_QS_fnc_iconUnits,
	_QS_fnc_onMapSingleClick,_QS_fnc_mapVehicleShowCrew,_QS_fnc_iconDrawMap,_QS_fnc_iconDrawGPS,_QS_fnc_groupIconText,
	_QS_fnc_groupIconType,_QS_fnc_configGroupIcon,_QS_fnc_onGroupIconClick,_QS_fnc_onGroupIconOverLeave,_QS_ST_iconMapClickShowDetail,
	_QS_ST_showFriendlySides,_QS_fnc_onGroupIconOverEnter,_QS_ST_showCivilianGroups,_QS_ST_iconTextFont,_QS_ST_showAll,
	_QS_ST_showFactionOnly,_QS_ST_showAI,_QS_ST_showMOS,_QS_ST_showGroupOnly,
